...
def main():
    ...
    if current_data is not None and not current_data.empty:
        # 初始化分页状态（使用独立的状态键以避免冲突）
        if 'page_13' not in st.session_state:
            st.session_state.page_13 = 1
        ...
        render_product_detail_table(
            filtered_data,
            prev_data,
            page=st.session_state.page_13,  # 使用独立的分页状态
            page_size=page_size,
        ...
        with col1:
            # 首页按钮
            if st.button("首页", key="first_page_13"):
                st.session_state.page_13 = 1

        with col2:
            # 上一页按钮
            if st.button("上一页", key="prev_page_13") and st.session_state.page_13 > 1:
                st.session_state.page_13 -= 1
        ...
        with col3:
            ...
            with jump_col1:
                target_page = st.text_input(
                    "跳至页码",
                    str(st.session_state.page_13),
                    key="page_input_13"
                )
            with jump_col2:
                if st.button("跳转", key="go_page_13"):
                    try:
                        page_num = int(target_page)
                        if 1 <= page_num <= total_pages:
                            st.session_state.page_13 = page_num
                    except ValueError:
                        st.warning("请输入有效的页码")

        with col4:
            # 下一页按钮
            if st.button("下一页", key="next_page_13") and st.session_state.page_13 < total_pages:
                st.session_state.page_13 += 1

        # 显示当前页码信息
        st.info(f"当前页: {st.session_state.page_13}/{total_pages} 页 | 每页 {page_size} 条 | 共 {total_items} 条记录")

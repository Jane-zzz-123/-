...
def load_and_preprocess_data(file_path):
    ...
    try:
        ...
        # 处理日期列
        df["预计FBA用完时间"] = pd.to_datetime(df["预计FBA用完时间"])
...
def render_product_detail_table(data, prev_data=None, page=1, page_size=30, table_id=""):
    """渲染产品风险详情表（带环比和分页功能，支持排序后的数据）"""
    if data is None or data.empty:
        st.markdown("<p style='color:#666'>无匹配产品数据</p>", unsafe_allow_html=True)
        return 0

    # 1. 定义要显示的列（保持原有逻辑，但使用传入的完整数据）
    display_cols = [
        "MSKU", "品名", "店铺", "日均", "7天日均", "14天日均", "28天日均",
        "FBA+AWD+在途库存", "全部总库存", "预计FBA用完时间", "预计总库存用完",
        "状态判断", "FBA滞销数量", "本地滞销数量", "总滞销库存",
        "预计总库存需要消耗天数", "预计用完时间比目标时间多出来的天数", "状态变化情况"
    ]

    # 确保只选择数据中存在的列（但使用排序后的完整数据）
    available_cols = [col for col in display_cols if col in data.columns]
    table_data = data[available_cols].copy()  # 这里的data已经是排序后的完整数据
    total_rows = len(table_data)

    # 2. 计算分页（基于排序后的数据）
    total_pages = (total_rows + page_size - 1) // page_size  # 替代ceil函数，避免导入
    start_idx = (page - 1) * page_size
    end_idx = min(start_idx + page_size, total_rows)
    paginated_data = table_data.iloc[start_idx:end_idx].copy()

    # 3. 格式化日期（保持原有逻辑）
    date_cols = ["预计FBA用完时间", "预计总库存用完"]
    ...
    if prev_data is not None and not prev_data.empty:
        prev_map = prev_data.set_index("MSKU")[
            ["日均", "7天日均", "14天日均", "28天日均", "FBA+AWD+在途库存",
             "全部总库存", "FBA滞销数量", "本地滞销数量", "总滞销库存",
             "预计总库存需要消耗天数", "预计用完时间比目标时间多出来的天数"]
        ].to_dict("index")
...
def render_status_change_table(data, page=1, page_size=30):
    ...
    # 格式化日期
    date_cols = ["记录时间", "预计FBA用完时间", "预计总库存用完"]
    for col in date_cols:
        if col in paginated_data.columns:
            paginated_data[col] = pd.to_datetime(paginated_data[col]).dt.strftime("%Y-%m-%d")
    ...
    # 添加状态变化情况颜色
    if "状态变化情况" in paginated_data.columns:
        ...
        paginated_data["状态变化情况"] = paginated_data["状态变化情况"].apply(color_status_change)
...
def main():
    ...
    with st.sidebar:
        ...
        # 显示日期信息
        today = datetime.now().date()  # 直接使用导入的datetime类
        st.info(f"当前日期：{today.strftime('%Y年%m月%d日')}")
        ...
        - **低滞销风险**：预计用完时间比目标时间多出来的天数≤10天；
        - **中滞销风险**：预计用完时间比目标时间多出来的天数≤30天；
        - **高滞销风险**：预计用完时间比目标时间多出来的天数>30天。
        """)
    ...
    if current_data is not None and not current_data.empty:
        ...
        status_change_counts = {
            "健康": {"变好": 0, "不变": 0, "变差": 0},
            "低滞销风险": {"变好": 0, "不变": 0, "变差": 0},
            "中滞销风险": {"变好": 0, "不变": 0, "变差": 0},
            "高滞销风险": {"变好": 0, "不变": 0, "变差": 0}
        }
                    ...
                    if not prev_record.empty:
                        ...
                        if prev_status == status:
                            ...
                            if status_severity[status] < status_severity[prev_status]:
                                status_change_counts[status]["变好"] += 1
                            else:
                                status_change_counts[status]["变差"] += 1
        ...
        with cols[0]:
            ...
            # 计算总体状态变化统计
            total_better = sum(v["变好"] for v in status_change_counts.values())
            total_same = sum(v["不变"] for v in status_change_counts.values())
            total_worse = sum(v["变差"] for v in status_change_counts.values())

            change_text = f"状态变化:<br>变好: {total_better} 不变: {total_same} 变差: {total_worse}"
        ...
        with cols[1]:
            ...
            change_text = f"状态变化:<br><span style='color:#2E8B57'>变好: {change_counts['变好']}</span> <span style='color:#1E90FF'>不变: {change_counts['不变']}</span> <span style='color:#DC143C'>变差: {change_counts['变差']}</span>"
        ...
        # 第三个图表：状态变化情况柱状图（按变好/不变/变差着色）
        with col3:
            # 准备状态变化数据
            change_types = ["变好", "不变", "变差"]
            change_colors = {"变好": "#2E8B57", "不变": "#1E90FF", "变差": "#DC143C"}
    ...
    # 1.3 产品风险详情（排序筛选器优先级最高 + 保留所有列 + 分页）
    st.subheader("1.3 产品风险详情")
    if current_data is not None and not current_data.empty:
        # ----------------------
        # 1. 初始化会话状态（仅存储排序+分页核心状态，避免干扰）
        # ----------------------
        # 分页状态：默认第1页，每页30条（独立存储，不受排序影响）
        if "prod_detail_page" not in st.session_state:
            st.session_state.prod_detail_page = 1
        # 排序状态：默认无排序，仅记录指定排序列和方向（优先级最高）
        if "prod_detail_sort_col" not in st.session_state:
            st.session_state.prod_detail_sort_col = None  # 仅存储指定的6个排序列
        if "prod_detail_sort_dir" not in st.session_state:
            st.session_state.prod_detail_sort_dir = "asc"  # asc=升序，desc=降序

        # ----------------------
        # 2. 第一步：执行原有筛选（店铺+状态+关键词）
        # ----------------------
        # 原有筛选器（保持不变）
        col1, col2, col3 = st.columns(3)
        with col1:
            selected_store = st.selectbox(
                ...
                key="prod_filter_store"
            )
        with col2:
            selected_status = st.selectbox(
                "选择风险状态",
                options=["全部", "健康", "低滞销风险", "中滞销风险", "高滞销风险"],
                key="prod_filter_status"
            )
        with col3:
            search_msku = st.text_input(
                "搜索MSKU/品名",
                placeholder="输入关键词搜索",
                key="prod_filter_keyword"
            )

        # 应用原有筛选（保留所有列，仅过滤行）
        filtered_data = current_data.copy()  # 原始数据全列复制
        ...
        if search_msku:
            ...
            filtered_data = filtered_data[
                filtered_data["MSKU"].fillna("").str.lower().str.contains(search_str) |
                filtered_data["品名"].fillna("").str.lower().str.contains(search_str)
                ]

        # ----------------------
        # 3. 第二步：执行排序筛选器（优先级最高，在筛选后执行）
        # ----------------------
        st.markdown("### 排序设置（优先级最高）")
        sort_layout = st.columns(3)

        # 3.1 仅显示指定的6个可排序列（与需求一致）
        with sort_layout[0]:
            target_sort_columns = [
                "预计总库存用完",
                "FBA滞销数量",
            ...
            # 过滤数据中实际存在的列（避免报错）
            available_sort_cols = [col for col in target_sort_columns if col in filtered_data.columns]
            selected_sort_col = st.selectbox(
                "选择排序列",
                options=[""] + available_sort_cols,  # 空选项=不排序
                key="prod_sort_col_select",
                # 回显当前排序状态（增强用户感知）
                index=0 if not st.session_state.prod_detail_sort_col
                else (available_sort_cols.index(st.session_state.prod_detail_sort_col) + 1)
            )

        # 3.2 升序/降序按钮（点击立即更新排序状态，优先级最高）
        with sort_layout[1]:
            if st.button("升序", key="prod_sort_asc") and selected_sort_col:
                # 直接覆盖排序状态，忽略其他逻辑
                st.session_state.prod_detail_sort_col = selected_sort_col
                st.session_state.prod_detail_sort_dir = "asc"
                st.session_state.prod_detail_page = 1  # 排序后重置到第1页（符合用户习惯）

        with sort_layout[2]:
            if st.button("降序", key="prod_sort_desc") and selected_sort_col:
                # 直接覆盖排序状态，确保优先级
                st.session_state.prod_detail_sort_col = selected_sort_col
                st.session_state.prod_detail_sort_dir = "desc"
                st.session_state.prod_detail_page = 1  # 排序后重置分页

        # 3.3 执行排序（严格按排序筛选器状态，无其他干扰）
        final_data = filtered_data  # 初始为筛选后的数据
        if st.session_state.prod_detail_sort_col:
            # 仅按排序筛选器指定的列和方向排序，处理空值（放最后）
            final_data = filtered_data.sort_values(
                by=st.session_state.prod_detail_sort_col,
                ascending=(st.session_state.prod_detail_sort_dir == "asc"),
                na_position="last"  # 空值不影响排序结果顺序
            )
        ...
        # 分页边界控制（避免页码超出范围）
        if st.session_state.prod_detail_page < 1:
            st.session_state.prod_detail_page = 1
        elif st.session_state.prod_detail_page > total_pages:
            st.session_state.prod_detail_page = total_pages

        # 截取当前页数据（保留所有列）
        start_idx = (st.session_state.prod_detail_page - 1) * page_size
        end_idx = start_idx + page_size
        current_page_data = final_data.iloc[start_idx:end_idx]
        ...
        sort_status = ""
        if st.session_state.prod_detail_sort_col:
            sort_dir_text = "升序" if st.session_state.prod_detail_sort_dir == "asc" else "降序"
            sort_status = f"，当前按【{st.session_state.prod_detail_sort_col}】{sort_dir_text}排序（排序优先级最高）"
        st.write(f"筛选结果：共 {total_records} 条记录{sort_status}")

        # 渲染表格（全列显示，使用原有函数，确保样式一致）
        render_product_detail_table(
            current_page_data,  # 排序后的当前页数据（全列）
            prev_data,
            page=st.session_state.prod_detail_page,
            page_size=page_size,
            table_id="main_products"  # 唯一标识，避免与其他表格冲突
        )

        # 分页控制（独立页码，与排序状态绑定）
        page_controls = st.columns(3)
        with page_controls[0]:
            if st.button("上一页", key="prod_page_prev", disabled=st.session_state.prod_detail_page == 1):
                st.session_state.prod_detail_page -= 1
        with page_controls[1]:
            st.write(f"第 {st.session_state.prod_detail_page} / {total_pages} 页")
        with page_controls[2]:
            if st.button("下一页", key="prod_page_next", disabled=st.session_state.prod_detail_page == total_pages):
                st.session_state.prod_detail_page += 1

        # 下载（排序后的全列数据，包含所有筛选+排序结果）
        if not final_data.empty:
            csv = final_data.to_csv(index=False, encoding="utf-8-sig")
            # 文件名包含排序信息（明确优先级）
            store_part = selected_store if selected_store != "全部" else "所有店铺"
            status_part = selected_status if selected_status != "全部" else "所有状态"
            search_part = f"_搜索{search_msku}" if search_msku else ""
            sort_part = f"_排序{st.session_state.prod_detail_sort_col}{st.session_state.prod_detail_sort_dir}" if st.session_state.prod_detail_sort_col else ""
            file_name = f"产品风险详情_{store_part}_{status_part}{search_part}{sort_part}_{pd.Timestamp.now().strftime('%Y%m%d')}.csv"
    ...
    if current_data is not None and not current_data.empty:
        ...
        if selected_store:
            ...
            if last_week_data is not None and not last_week_data.empty and "MSKU" in store_current_data.columns:
                ...
                for _, row in merged_data.iterrows():
                    ...
                    if current_status == prev_status:
                        ...
                        status_change[current_status]["变好"] += 1  # 当前状态更轻=变好
                    else:
                        status_change[current_status]["变差"] += 1  # 当前状态更重=变差
            ...
            def get_status_change_text(status):
                ...
                total = changes["变好"] + changes["不变"] + changes["变差"]
                if total == 0:
                    return "<br><span style='color:#666; font-size:0.8em;'>状态变化: 无数据</span>"

                return f"""<br>
                <span style='color:#2E8B57; font-size:0.8em;'>变好: {changes['变好']}</span> | 
                <span style='color:#666; font-size:0.8em;'>不变: {changes['不变']}</span> | 
                <span style='color:#DC143C; font-size:0.8em;'>变差: {changes['变差']}</span>
                """
            ...
            # 1. 初始化排序会话状态
            # ----------------------
            if "single_store_sort_col" not in st.session_state:
                st.session_state.single_store_sort_col = None
            if "single_store_sort_dir" not in st.session_state:
                st.session_state.single_store_sort_dir = "asc"

            # ----------------------
            # 2. 定义状态判断的自定义排序优先级（核心：风险越高优先级越高）
            # ----------------------
            status_sort_order = {
                "高滞销风险": 4,
                "中滞销风险": 3,
            ...
            with sort_layout[0]:
                ...
                available_sort_cols = [col for col in target_sort_cols if col in store_current_data.columns]
                selected_sort_col = st.selectbox(
                    "选择排序列",
                    options=[""] + available_sort_cols,
                    key=f"single_store_sort_col_{selected_store}",
                    index=0 if not st.session_state.single_store_sort_col
                    else (available_sort_cols.index(st.session_state.single_store_sort_col) + 1)
                )

            # 3.2 选择排序方向
            with sort_layout[1]:
                selected_sort_dir = st.radio(
                    "排序方向",
                    options=["升序", "降序"],
                    key=f"single_store_sort_dir_{selected_store}",
                    horizontal=True,
                    index=0 if st.session_state.single_store_sort_dir == "asc" else 1
                )

            # 3.3 应用排序按钮
            with sort_layout[2]:
                if st.button("应用排序", key=f"single_store_apply_sort_{selected_store}"):
                    st.session_state.single_store_sort_col = selected_sort_col
                    st.session_state.single_store_sort_dir = "asc" if selected_sort_dir == "升序" else "desc"
                    st.session_state.current_page = 1  # 排序后重置到第1页

            # ----------------------
            # 4. 执行排序（核心：对状态判断列用自定义顺序，其他列用默认排序）
            # ----------------------
            sorted_full_data = store_current_data.copy()  # 保留所有列
            if st.session_state.single_store_sort_col and st.session_state.single_store_sort_col in sorted_full_data.columns:
                sort_col = st.session_state.single_store_sort_col
                sort_dir = st.session_state.single_store_sort_dir == "asc"  # True=升序，False=降序

                # 4.1 若排序列是“状态判断”：用自定义优先级排序
                if sort_col == "状态判断":
                    # 新增临时列：存储状态对应的优先级数值
                    sorted_full_data["_status_priority"] = sorted_full_data["状态判断"].map(status_sort_order)
                    # 按优先级排序（降序时优先级高的在前）
                    sorted_full_data = sorted_full_data.sort_values(
                        by="_status_priority",
                        ascending=sort_dir,  # 升序=健康→高风险；降序=高风险→健康
                        na_position="last"  # 空值放最后
                    )
                    # 删除临时列（不影响原数据）
                    sorted_full_data = sorted_full_data.drop(columns=["_status_priority"])

                # 4.2 其他列：用默认排序（数值/日期/字符串默认规则）
                else:
                    sorted_full_data = sorted_full_data.sort_values(
                        by=sort_col,
                        ascending=sort_dir,
            ...
            # ----------------------
            sort_status_text = ""
            if st.session_state.single_store_sort_col:
                dir_text = "升序" if st.session_state.single_store_sort_dir == "asc" else "降序"
                sort_status_text = f"，按【{st.session_state.single_store_sort_col}】{dir_text}排序"
            st.write(f"筛选结果：共 {len(sorted_full_data)} 条记录{sort_status_text}")

            # 渲染表格（无多余参数，传递排序后的完整数据）
            render_product_detail_table(
                data=sorted_full_data,
                prev_data=prev_data[prev_data["店铺"] == selected_store] if (
            ...
            # 6. 下载功能（不变）
            # ----------------------
            if not sorted_full_data.empty:
                download_data = sorted_full_data.copy()
                # 日期格式化
                date_cols = ["记录时间", "预计FBA用完时间", "预计总库存用完"]
                for col in date_cols:
                ...
                # 构建文件名
                sort_info = f"_按{st.session_state.single_store_sort_col}{st.session_state.single_store_sort_dir}" if st.session_state.single_store_sort_col else ""
                file_name = f"{selected_store}_产品列表_{today.strftime('%Y%m%d')}{sort_info}.csv"
                # 下载按钮
                st.download_button(
                    label="下载筛选结果 (CSV)",
                    data=csv,
    ...
    if df is not None and not df.empty:
        ...
        display_columns = [
            ...
            "28天日均","FBA+AWD+在途库存","全部总库存","预计FBA用完时间",
            "预计总库存用完","状态判断",
             "预计总库存需要消耗天数", "FBA滞销数量",
            "本地滞销数量", "总滞销库存",
            "预计总库存需要消耗天数", "预计用完时间比目标时间多出来的天数","状态变化情况"
        ...
        if not analysis_data.empty:
            # 准备要下载的数据（只包含显示的列）
            download_data = analysis_data[display_columns].copy()
    ...
    if df is not None and not df.empty:
        ...
        if selected_analysis_msku:
            ...
            # 添加状态变化情况颜色
            if "状态变化情况" in table_data.columns:
                def color_status_change(x):
                    if x == "变好":
                        return f"<span style='color:#2E8B57; font-weight:bold;'>{x}</span>"
                    elif x == "变差":
                ...
                table_data["状态变化情况"] = table_data["状态变化情况"].apply(color_status_change)

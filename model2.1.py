...
TARGET_DATE = datetime(2025, 11, 15)  # 目标消耗完成日期
...
def main():
    ...
    with st.sidebar:
        ...
        st.subheader("数据加载中...")
        try:
            # 替换为你的数据文件在GitHub的raw地址
            data_url = "https://github.com/Jane-zzz-123/-/upload/main/年份品-每周周一日均-补货建议 - 公式计算版.xlsx"
            ...
            # 读取Excel文件（根据你的实际sheet名调整）
            current_data = pd.read_excel(excel_data, sheet_name="当前数据")
            prev_data = pd.read_excel(excel_data, sheet_name="历史数据")  # 如果有历史数据
    ...
    if current_data is not None and not current_data.empty:
        ...
        # 3. 计算差异值和百分比变化（全部转为数值类型）
        metrics_data = {}
        for metric in ["总MSKU数", "健康", "低滞销风险", "中滞销风险", "高滞销风险"]:
            current_val = int(total_metrics[metric])  # 强制转为整数
            ...
            if last_week_val == 0:
                ...
                pct_change = float((diff / last_week_val) * 100)  # 强制转为浮点数
        ...
        def generate_compare_text(metric_data, metric_name):
            ...
            if metric_name == "总MSKU数":
                return f"<br><span style='color:{color}; font-size:0.8em;'>{trend} 上周{last_week_val}，变化{diff} ({pct:.1f}%)</span>"
            else:
                status_desc = "上升" if diff > 0 else "下降" if diff < 0 else "无变化"
                return f"<br><span style='color:{color}; font-size:0.8em;'>{trend} 上周{last_week_val}，{status_desc}{abs(diff)} ({pct:.1f}%)</span>"

        # 5. 渲染指标卡片（传递所有数值参数，确保类型正确）
        cols = st.columns(5)
        ...
        with cols[0]:
            ...
            render_metric_card(
                f"总MSKU数{compare_text}",  # 标题
                metric_data["current"],  # 当前值（整数）
                metric_data["diff"],  # 差异值（整数）
                metric_data["pct"],  # 百分比（浮点数）
                "#000000"  # 颜色
            )
        ...
        with col2:
            today = current_data["记录时间"].iloc[0]
            days_to_target = (TARGET_DATE - today).days
    ...
    if current_data is not None and not current_data.empty:
        ...
        if not filtered_data.empty:
            # 准备要下载的数据（只包含显示的列）
            download_data = filtered_data[display_columns]
        ...
        if selected_store:
            ...
            def get_store_last_week_metrics():
                ...
                if 'prev_data' in locals() and prev_data is not None and not prev_data.empty:
                    ...
                    if not last_week_data.empty:
                        return calculate_status_metrics(last_week_data)

                return {"总MSKU数": 0, "健康": 0, "低滞销风险": 0, "中滞销风险": 0, "高滞销风险": 0}

            store_last_week_metrics = get_store_last_week_metrics()
            ...
            def get_compare_text(metric_data, metric_name):
                ...
                if metric_name == "总MSKU数":
                    return f"<br><span style='color:{color}; font-size:0.8em;'>{trend} 上周{metric_data['last_week']}，变化{metric_data['diff']} ({metric_data['pct']}%)</span>"
                else:
                    status = "上升" if metric_data["diff"] > 0 else "下降" if metric_data["diff"] < 0 else "无变化"
                    return f"<br><span style='color:{color}; font-size:0.8em;'>{trend} 上周{metric_data['last_week']}，{status}{abs(metric_data['diff'])} ({metric_data['pct']}%)</span>"
            ...
            with cols[0]:
                ...
                render_metric_card(
                    f"{selected_store} 总MSKU数{compare_text}",
                    data["current"],
                    data["diff"],
                    data["pct"],
                    "#000000"
            ...
            with cols[1]:
                ...
                render_metric_card(
                    f"{selected_store} 健康{compare_text}",
                    data["current"],
                    data["diff"],
                    data["pct"],
                    STATUS_COLORS["健康"]
            ...
            with cols[2]:
                ...
                render_metric_card(
                    f"{selected_store} 低滞销风险{compare_text}",
                    data["current"],
                    data["diff"],
                    data["pct"],
                    STATUS_COLORS["低滞销风险"]
            ...
            with cols[3]:
                ...
                render_metric_card(
                    f"{selected_store} 中滞销风险{compare_text}",
                    data["current"],
                    data["diff"],
                    data["pct"],
                    STATUS_COLORS["中滞销风险"]
            ...
            with cols[4]:
                ...
                render_metric_card(
                    f"{selected_store} 高滞销风险{compare_text}",
                    data["current"],
                    data["diff"],
                    data["pct"],
                    STATUS_COLORS["高滞销风险"]
            ...
            with col2:
                ...
                fig_days = px.bar(
                    ...
                    y="MSKU数量",
                    color_discrete_sequence=["#87CEEB"],
                    title=f"{selected_store} 库存消耗天数分布",
                    text="MSKU数量",
                    height=400
                )
    ...
    if current_data is not None and not current_data.empty:
        ...
        selected_msku = st.selectbox("选择MSKU查看详情", options=msku_list)

...
def load_and_preprocess_data_from_df(df):
    ...
    try:
        # 1. 时间段系数配置（您已定义，保持不变）
        TIME_PERIODS = [
            {"name": "december", "start": datetime(2025, 12, 1), "end": datetime(2025, 12, 31), "coefficient": 0.60}
        ]
        ...
        df["12月1-31日调整后日均"] = (df["日均"] * 0.60).round(2)
        ...
        def calculate_exhaust_date(row, stock_col):
            ...
            phase1_end = datetime(2025, 11, 30)
...
def render_stock_forecast_chart(data, msku):
    ...
    TIME_PERIODS = [
        {"start": datetime(2025, 12, 1), "end": datetime(2025, 12, 31), "coefficient": 0.60}
    ]
